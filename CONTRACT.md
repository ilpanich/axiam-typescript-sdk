...
- **§24.6 — the platform ceremony.** Driving an authenticator. Only where the
  runtime has one; §24.7 records who does and who does not.
...
### §24.6 The platform ceremony (only where an authenticator API exists)

An SDK on a runtime with an authenticator API SHOULD expose the three composed
helpers of §24.1, each of which is: call `*_start`, run the ceremony, call
`*_finish`.

1. **The composed helper is additive.** The six wire operations remain public.
   An SDK MUST NOT make the ceremony the only way to reach them — a caller running
   a virtual authenticator in a test, or holding a response produced elsewhere,
   needs the pair directly. This is §22.14 rule 1's shape and it is the same
   argument.
2. **It MUST NOT be offered where there is no authenticator.** An SDK on a
   runtime without one MUST NOT ship a helper that fabricates a ceremony, and MUST
   NOT emulate an authenticator in software. A software "authenticator" holding a
   key in process memory is not a second factor; shipping one under this section's
   name would make an SDK the weakest link in a mechanism chosen for being the
   strongest.
3. **Conditional mediation** (passkey autofill) SHOULD be exposed as a flag on the
   discoverable helper where the platform supports it, and MUST degrade to the
   explicit prompt where it does not. The probe for it MUST NOT throw on platforms
   that lack it. A ceremony in this mode may never settle — the user simply may
   not pick a passkey — so the SDK MUST allow the caller to abandon it and MUST NOT
   surface an abandoned conditional ceremony as an authentication failure.
4. **`authenticatorAttachment` is the one permitted addition** to the server's
   ...
   MUST NOT infer it and MUST NOT default it to a value.
5. **The five ceremony failures are classified, not passed through raw.** Every
   ...
   is a name, and translating that in one place beats translating it in every
   caller:
   ...
   **`cancelled` covers both an explicit refusal and a silent timeout, and this is
   ...
   authenticator was present. An SDK MUST NOT try to recover the distinction by
   timing the call, and its user-facing copy MUST NOT accuse the user of
   cancelling.
...
6. **Feature detection is a query, not an exception.** An SDK exposing the
   ceremony MUST expose a predicate for whether the runtime can perform one, so a
   caller can hide a button rather than offer one that throws.
...
| `webauthn_register` | — | `webauthnRegister` | — | — | — | — | — | — | `webauthnRegister` | — | — |
| `webauthn_login` | — | `webauthnLogin` | — | — | — | — | — | — | `webauthnLogin` | — | — |
| `webauthn_discoverable_login` | — | `webauthnDiscoverableLogin` | — | — | — | — | — | — | `webauthnDiscoverableLogin` | — | — |
...
**Who ships the §24.6 ceremony, and why the line falls where it does.** An
authenticator API is a property of the *runtime*, not of the language, which is
why this table does not follow any of the others in this contract:

| SDK | Relying-party layer | Platform ceremony | Reason |
|---|---|---|---|
| TypeScript | yes | **yes** — `navigator.credentials` | The browser build is the archetype. The Node build exposes the RP layer only; the ceremony helpers are absent there rather than throwing. |
| Rust (`axiam-sdk-wasm`) | yes | **yes** — `navigator.credentials` via `web-sys` | Same runtime as TypeScript's browser build, reached through WASM. |
| Swift | yes | **yes** — `AuthenticationServices` | `ASAuthorizationPlatformPublicKeyCredentialProvider`, behind `#if canImport(AuthenticationServices)` plus an OS-version guard. Absent on the Linux build, which is a supported target of this SDK. |
| Rust (native), Python, Java, Kotlin, C#, PHP, Go, C, C++ | yes | **no** | No authenticator on the runtime. Not a deferral and not a gap: §24.6 rule 2 forbids inventing one. These SDKs are the relying party for a ceremony that ran somewhere else. |

**Kotlin is not Android**, and this is the row most likely to be read wrong.
`axiam-kotlin-sdk` is a `kotlin("jvm")` library; Android's Credential Manager is
not on its classpath and adding an Android target to reach it would change what the
artifact is. A Kotlin *Android app* runs the ceremony with Credential Manager and
hands the response to this SDK's `webauthnAuthenticateFinish` — which is the RP
layer working exactly as intended, and is worth an explicit paragraph in that
SDK's README because the alternative reading is "Kotlin can't do passkeys".
...
**Platform ceremony — the three SDKs that ship it:**

- Each of the five §24.6 rule 5 platform errors maps to its canonical
  classification, and `already_registered` is distinguishable from `cancelled`.
