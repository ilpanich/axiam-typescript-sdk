...
This SDK conforms to **contract 1.49**: CONTRACT.md §1–§13 and §12.7, §14, §15, §17, §19,
...
A TLS listener decides whether to ask for a client certificate during the handshake,
before it has seen a byte of HTTP, so "request one on `/oauth2/token` but not on
`/oauth2/authorize`" is not something a single listener can do. A deployment that wants
both runs two — and `mtls_endpoint_aliases` in the discovery document is how the second
one is named.

Once the client above is built with a `clientCert`, every request it makes presents that
...
|---|---|
| `oidcExchange`, `oidcRefresh`, `loginClientCredentials`, `tokenExchange`, `umaExchangeTicket` | `token_endpoint` |
...
- **An absent `mtls_endpoint_aliases` is not an error.** It means "this deployment
...
### `dpop_jkt` (RFC 9449 §10.1, contract 1.42)

`oidcPar` accepts an optional `dpopJkt`, pushed as `dpop_jkt` **only when you set
it** and omitted entirely otherwise:

```ts
const pushed = await oidc.oidcPar({
  request, redirectUri, configuration,
  dpopJkt: jwkThumbprintS256(myDpopPublicJwk),   // exported from axiam-sdk/node
});
```

Without it the authorization code is bound to a DPoP key only at the token
request, so an attacker who steals the code can redeem it with a key of their
own. RFC 9449 §10 binds the code to the key at authorization time instead —
which is the point of pushing the request over the back channel in the first
place.

**You supply the value; the SDK does not derive it.** This SDK implements the
resource-server half of DPoP (§21.7.2 proof *verification*) and ships no proof
generator, so it holds no DPoP key whose thumbprint it could take.
`jwkThumbprintS256` is exported for the client that does, and produces exactly
the RFC 7638 value this parameter wants.

**`request_uri` is not on this surface and will not be added.** RFC 9126 §2.1
makes it the one authorization parameter a client MUST NOT push; `openapi.json`
models it on the request schema so the *server* can refuse it. A client able to
send one is a client able to chain one pushed request into another.
